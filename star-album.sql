...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `zone_view` AS select `user`.`id` AS `id`,`user`.`name` AS `name`,`user`.`portrait` AS `portrait`,`user`.`follow` AS `follow`,`user`.`fans` AS `fans`,`user`.`followed_user` AS `followed_user`,`user`.`fans_user` AS `fans_user`,`user`.`like_album` AS `like_album`,`user`.`collect_album` AS `collect_album`,`album`.`album_id` AS `album_id`,`album`.`cover` AS `cover`,`album`.`album_name` AS `album_name`,`album`.`type` AS `type`,`album`.`date` AS `date`,`album`.`img` AS `img`,`album`.`img_name` AS `img_name`,`album`.`likes` AS `likes`,`album`.`views` AS `views`,`album`.`collect` AS `collect` from (`user` join `album` on((`user`.`id` = `album`.`user_id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2020-02-21 13:52:48
